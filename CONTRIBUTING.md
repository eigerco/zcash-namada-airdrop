# Contributing to `zcash-namada-airdrop` Crates
...
> If you want to ask a question, we assume that you have read the available documentation for the crate or crates in question.
...
that the maintainers of `zcash-namada-airdrop` have a relatively heavy workload, so
...
`zcash-namada-airdrop` is a Rust workspace containing several interdependent crates. Please refer to the
[README](README.md) for a summary documentation for each module and the [USAGE](./docs/USAGE.md) for usage examples of the provided crates.
...
will be considered for application against the last versions.
...
This repository is currently developed with a "unstable main" workflow. The
...
- If the modification can be applied as a SemVer-compatible change without
  ...
  most recent tag in the series from the _previous_ SemVer major release
...
The `zcash-namada-airdrop` authors hold our software to a high standard of quality. The
...
In `zcash-namada-airdrop` code, type safety is of paramount importance. This has
...
The public API of the `zcash-namada-airdrop` crates is carefully curated. We rely on
...
- When using derived serialization (e.g., `serde`), types that are
  serialization-critical should be clearly documented, and these types may not
  be modified in backwards-incompatible ways once they have been exposed in a
  public release of the associated crate. The data serialized by derivation-based

...
use bls12_381::Bls12;
use claim_circuit::{
    ClaimParameters, ClaimProofInputs, ClaimProofOutput, generate_claim_proof, generate_parameters,
    load_parameters, save_parameters, verify_claim_proof_output,
};
...
///
/// This struct extends [`claim_circuit::ClaimProofOutput`] with metadata (`value`, `block_height`)
/// needed for airdrop claim submission. The proof fields (`zkproof`, `rk`, `cv`, `anchor`,
/// `nm_anchor`) mirror `ClaimProofOutput`, while `hiding_nullifier` corresponds to
...
/// Verify all Sapling claim proofs from a proofs file (output of generate-claim-proofs).
///
/// # Arguments
///
...
pub async fn verify_claim_sapling_proof(
    ...
    // Load proofs from JSON (ClaimProofsOutput format from generate-claim-proofs)

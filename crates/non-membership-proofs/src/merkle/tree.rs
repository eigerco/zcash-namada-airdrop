...
use crate::{MAX_NF, MIN_NF, Nullifier};
...
///
/// The [`NonMembershipNode::from_nullifiers`] method is also available be used. It builds the tree
...
fn gap_bounds(nullifiers: &[Nullifier], gap_idx: usize) -> Gap {
    ...
    // Empty slice: single gap (MIN_NF, MAX_NF)
    if len == 0 {
        return Gap {
            left: MIN_NF,
            right: MAX_NF,
...
mod tests {
    ...
    mod from_chain_and_user_nullifiers {
        ...
        fn user_in_first_gap() {
            ...
            assert_eq!(
                mapping[0],
                TreePosition::new(nf!(25), 0, MIN_NF, nf!(50)).expect("Position creation failed")
            ); // gap index 0
        ...
        fn user_in_last_gap() {
            ...
            assert_eq!(
                mapping[0],
                TreePosition::new(nf!(200), 2, nf!(20), MAX_NF).expect("Position creation failed")
            ); // gap index 2
        ...
        fn multiple_users_different_gaps() {
            ...
            assert_eq!(
                mapping[0],
                TreePosition::new(nf!(5), 0, MIN_NF, nf!(10)).expect("Position creation failed")
            );
            assert_eq!(
                mapping[1],
                TreePosition::new(nf!(15), 1, nf!(10), nf!(20)).expect("Position creation failed")
            ...
            assert_eq!(
                mapping[3],
                TreePosition::new(nf!(200), 3, nf!(30), MAX_NF).expect("Position creation failed")
            );
        ...
        fn empty_chain_nullifiers() {
            ...
            assert_eq!(mapping[0].left_bound, MIN_NF);
            assert_eq!(mapping[0].right_bound, MAX_NF);
        }
